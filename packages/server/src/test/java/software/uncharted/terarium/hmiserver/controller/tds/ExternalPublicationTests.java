...
public class ExternalPublicationTests extends TerariumApplicationTests {
	...
	public void testCanItCreateGetUpdateDeletePublication() throws Exception {
		...
		ExternalPublication publication = new ExternalPublication().setXddUri(SAMPLE_GDDID).setTitle(SAMPLE_TITLE);
		...
		try {
			JsonNode responseJson = mapper.readValue(result.getResponse().getContentAsByteArray(), JsonNode.class);
