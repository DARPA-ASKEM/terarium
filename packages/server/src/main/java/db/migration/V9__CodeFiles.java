	...
	public void migrate(final Context context) throws Exception {
		try (var statement = context.getConnection().createStatement()) {
			...
			while (resultSet.next()) {
				...
				final Map<String, CodeFile> codeFiles =
						objectMapper.readValue(codeFileString, new TypeReference<Map<String, CodeFile>>() {});
				...
				for (final Map.Entry<String, CodeFile> entry : codeFiles.entrySet()) {
					...
					preparedStatement.setString(
							4, file.getDynamics() != null ? objectMapper.writeValueAsString(file.getDynamics()) : null);
					preparedStatement.setString(5, fileName);
					preparedStatement.setObject(
							6, file.getLanguage() != null ? file.getLanguage().ordinal() : null);
