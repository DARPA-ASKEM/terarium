...
public class ProjectController {
  ...
  public ResponseEntity<List<Project>> getProjects(
      @RequestParam(name = "include-inactive", defaultValue = "false")
    ...
    projects.forEach(
        project -> {
          try {
            ...
            metadata.put(
                "publications-count", counts.getOrDefault(AssetType.PUBLICATION, 0).toString());

            project.setMetadata(metadata);
          } catch (final Exception e) {
            log.error(
                "Cannot get Datasets, Models, and Publications assets from data-service for"
                    + " project_id {}",
                project.getId(),
                e);
          }
   ...
   * @return The project wrapped in a response entity, a 404 if missing or a 500 if there is a rebac
   *     permissions issue.
   */
  ...
  public ResponseEntity<ResponseDeleted> deleteProject(@PathVariable("id") final UUID id) {
    ...
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to delete project");
    }

    throw new ResponseStatusException(
        HttpStatus.NOT_MODIFIED, "Unable to delete project, please try again later.");
  }
...
""";
    ...
    try {
      final RebacProject rebacProject = new RebacProject(project.getId(), reBACService);
      final RebacGroup rebacAskemAdminGroup =
          new RebacGroup(ReBACService.ASKEM_ADMIN_GROUP_ID, reBACService);
      final RebacUser rebacUser = new RebacUser(currentUserService.get().getId(), reBACService);
  ...
  public ResponseEntity<Project> updateProject(
    ...
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to update project");
    }
  }
    ...
    try {
      ...
      if (rebacUser.canWrite(rebacProject)) {
        ...
        if (project.isPresent()) {
          ...
          // double check that this asset is not already a part of this project, and if it does
          // exist return a 409 to the front end
          ...
          final TerariumAssetService<? extends TerariumAsset> terariumAssetService =
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to create project asset");
    }
  }
  ...
  public ResponseEntity<ResponseDeleted> deleteAsset(
    ...
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to delete project asset");
    }
  }
