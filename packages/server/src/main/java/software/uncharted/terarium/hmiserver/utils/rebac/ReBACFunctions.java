...
public class ReBACFunctions {
	...
	public boolean checkPermission(SchemaObject subject, Schema.Permission permission, SchemaObject resource,
			Consistency consistency) throws Exception {
		return checkPermission(subject.type.toString(), subject.id, permission.toString(), resource.type.toString(),
				resource.id, consistency);
	}

	public boolean checkPermission(String subjectType, String subjectId, String permission, String resourceType,
			String resourceId, Consistency consistency) throws Exception {
	...
	public String createRelationship(SchemaObject subject, Schema.Relationship relationship, SchemaObject target)
			throws Exception, RelationshipAlreadyExistsException {
		return createRelationship(subject.type.toString(), subject.id, relationship.toString(), target.type.toString(),
				target.id);
	}

	public String createRelationship(String subjectType, String subjectId, String relationship, String targetType,
			String targetId) throws Exception, RelationshipAlreadyExistsException {
	...
	public String removeRelationship(SchemaObject subject, Schema.Relationship relationship, SchemaObject target)
			throws Exception, RelationshipAlreadyExistsException {
		return removeRelationship(subject.type.toString(), subject.id, relationship.toString(), target.type.toString(),
				target.id);
	}

	private static final String ALREADY_EXISTS_CREATE_RELATIONSHIP = "ALREADY_EXISTS: could not CREATE relationship";

	public String removeRelationship(String subjectType, String subjectId, String relationship, String targetType,
			String targetId) throws Exception, RelationshipAlreadyExistsException {
	...
	public List<RebacPermissionRelationship> getRelationship(SchemaObject resource, Consistency consistency)
			throws Exception {
	...
	public List<RebacPermissionRelationship> getRelationship(PermissionService.ReadRelationshipsRequest request)
			throws Exception {
		...
		while (iter.hasNext()) {
			...
			RebacPermissionRelationship rebacRelationship = new RebacPermissionRelationship(subject,
					response.getRelationship().getRelation(), resource);
			relationships.add(rebacRelationship);
		}
		return relationships;
	}

	public boolean hasRelationship(SchemaObject who, Schema.Relationship relationship, SchemaObject what,
			Consistency consistency) throws Exception {
		PermissionService.ReadRelationshipsRequest request = PermissionService.ReadRelationshipsRequest.newBuilder()
				.setConsistency(consistency)
				.setRelationshipFilter(
	...
	public List<UUID> lookupResources(Schema.Type resourceType, Schema.Permission permission, SchemaObject who,
			Consistency consistency) throws Exception {
		...
		while (iter.hasNext()) {
			...
			if (response
					.getPermissionshipValue() == PermissionService.LookupPermissionship.LOOKUP_PERMISSIONSHIP_HAS_PERMISSION
							.getNumber()) {
				try {
