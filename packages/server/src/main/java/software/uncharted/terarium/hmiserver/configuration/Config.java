...
public class Config {
  ...
  /**
   * A list of unauthenticated {@link org.springframework.util.AntPathMatcher} patterns for urls
   * that should not be authenticated via Spring Security
   */
  ...
  public static class ClientConfig implements Serializable {
    ...
    /**
     * If true, we will log all client-side errors to the server. This is useful for debugging, but
     * should be false
     */
    ...
    /**
     * The interval, in milliseconds, at which we will send a heartbeat to connected clients for
     * server-side-events
     */
