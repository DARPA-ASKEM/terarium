...
/**
 * A class to handle scenario extraction from Document and Dataset via GoLLM task-runner. GoLLM
 * responses for the model configuration are in different JSON form depending on the source:
 * Document:{ "conditions": [ { "name": condition_name, "description": description, "initials": [ {
 * "id": "initial_id", "value": 0.0 }, ... ], "parameters": [ { "id": "parameter_id", "value": 0.0
...
public class ScenarioExtraction {
  ...
  public static void replaceParameter(
      final ModelParameter parameter, final JsonNode conditionParameter) {
    final String id = conditionParameter.get("id").asText();
    if (parameter.getId().equals(id)
        || parameter.getId().equals(GreekDictionary.englishToGreek(id))) {
  ...
  public static List<ModelParameter> getModelParameters(
      final JsonNode condition, final Model modelCopy) {
