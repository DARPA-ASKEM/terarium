...
public class SimulationController {
  ...
  public ResponseEntity<Simulation> getSimulation(@PathVariable("id") final UUID id) {
    try {
      ...
      if (simulation.isPresent()) {
        ...
        // If the simulation failed, then set an error message for the front end to display nicely.
        // We want to save this to the simulaiton object
  ...
  @Operation(
      summary =
          "Create a new dataset from a simulation result, then add it to a project as a Dataset")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "201",
  ...
  public ResponseEntity<ProjectAsset> createFromSimulationResult(
    ...
    try {
      ...
      final String error =
          String.format(
              "Failed to add simulation %s result as dataset to project %s", id, projectId);
      log.error(error, e);
      throw new ResponseStatusException(
          org.springframework.http.HttpStatus.INTERNAL_SERVER_ERROR, error);
    }
