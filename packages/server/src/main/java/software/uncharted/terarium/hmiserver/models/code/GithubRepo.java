...
/**
 * Github repository This is a convenience class for the front end to use to display a github
 * repository. It contains a list of files, and a count of the total number of files in the repo.
 */
@TSModel
public class GithubRepo {
  ...
  private static final List<String> DOCUMENT_TYPES =
      new ArrayList<>(List.of(".pdf", ".txt", ".md"));
  ...
  public GithubRepo(List<GithubFile> files) {
    ...
    // Add data to our map by iterating over our given files and adding them to the appropriate
    // category
    ...
    this.totalFiles =
        this.files.values().stream().mapToInt(List::size).sum()
