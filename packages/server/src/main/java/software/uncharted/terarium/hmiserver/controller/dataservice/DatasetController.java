...
public class DatasetController {
  ...
  public ResponseEntity<List<Dataset>> getDatasets(
      @RequestParam(name = "page-size", defaultValue = "100", required = false)
      ...
      if (!ts.isEmpty()) {
        ...
        for (final String field : SEARCH_FIELDS) {
          ...
          final TermsQuery termsQuery =
              new TermsQuery.Builder().field(field).terms(termsQueryField).build();
  ...
  public ResponseEntity<Dataset> createDataset(@RequestBody final Dataset dataset) {
    ...
    try {
      return ResponseEntity.status(HttpStatus.CREATED).body(datasetService.createAsset(dataset));
    } catch (final IOException e) {
   ...
   */
  private Optional<Dataset> extractColumnsAsNeededAndSave(final Optional<Dataset> dataset)
      throws IOException {
    if (dataset.isEmpty()) {
  ...
  ResponseEntity<Dataset> updateDataset(
      @PathVariable("id") final UUID id, @RequestBody final Dataset dataset) {
  ...
  public ResponseEntity<CsvAsset> getCsv(
    ...
    for (int i = 0; i < csv.get(0).size(); i++) {
      ...
      csvColumnStats.add(
          getStats(column.subList(1, column.size()))); // remove first as it is header:
    }

    final int linesToRead = limit != null ? (limit == -1 ? csv.size() : limit) : DEFAULT_CSV_LIMIT;

    final CsvAsset csvAsset =
        new CsvAsset(
  ...
  private List<List<String>> getCSVFile(
      final String filename, final UUID datasetId, final Integer limit) throws IOException {
    String rawCSV = "";
  ...
  public ResponseEntity<PresignedURL> getDownloadURL(
    ...
    if (dataset.get().getEsgfId() != null
      ...
      String url =
          dataset.get().getDatasetUrls().stream()
  ...
  /**
   * Uploads a CSV file to the dataset. This will grab a presigned URL from TDS then push the file
   * to S3.
   *
  ...
  public ResponseEntity<ResponseStatus> uploadCsv(
    ...
    try {
      ...
      final HttpEntity csvEntity =
          new ByteArrayEntity(csvBytes, ContentType.APPLICATION_OCTET_STREAM);
  ...
  public ResponseEntity<Void> uploadData( // HttpServletRequest request,
    ...
    try {
      ...
      if (res.getStatusCode() == HttpStatus.OK) {
        ...
        datasetService.updateAsset(updatedDataset.get());
      }
  ...
  /**
   * Uploads a CSV file to the dataset. This will grab a presigned URL from TDS then push the file
   * to S3 via a presigned URL and update the dataset with the headers.
   *
   * <p>If the headers fail to update there will be a print to the log, however, the response will
  ...
  private static void updateHeaders(final Dataset dataset, final List<String> headers) {
    ...
    for (final String header : headers) {
      final DatasetColumn column =
          new DatasetColumn().setName(header).setAnnotations(new ArrayList<>());
  ...
  private static List<List<String>> csvToRecords(final String rawCsvString) throws IOException {
    ...
    try (final CSVParser parser =
        new CSVParser(new StringReader(rawCsvString), CSVFormat.DEFAULT)) {
  ...
  private static CsvColumnStats getStats(final List<String> aCol) {
    ...
    try {
      ...
      final List<Double> numberList =
          aCol.stream().map(Double::valueOf).collect(Collectors.toList());
