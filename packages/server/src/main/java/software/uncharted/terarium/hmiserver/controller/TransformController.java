...
public class TransformController {
	...
	public ResponseEntity<JsonNode> mathML2ACSet(@RequestBody final List<String> list) {
		...
		return skemaProxy.convertMathML2ACSet(list);
			...
			throw new ResponseStatusException(
				error.status()< 100?  org.springframework.http.HttpStatus.INTERNAL_SERVER_ERROR: org.springframework.http.HttpStatus.valueOf(error.status()),
	...
	public ResponseEntity<JsonNode> model2Latex(@PathVariable("id") final UUID id) {
		try {
			...
			throw new ResponseStatusException(
				error.status()< 100?  org.springframework.http.HttpStatus.INTERNAL_SERVER_ERROR: org.springframework.http.HttpStatus.valueOf(error.status()),
				"Simulation Service is unable to provide the set of LaTeX equations from the Model. " + error.getMessage());
		}
