...
public class StructuredLog {
    ...
    /**
     * Prints a structure log message for the purpose of capturing in monitoring. If
     * JSON logging is not enabled, it
     * prints a more user friendly version on a single line separated by the "|"
     * character
     *
     * @param type          the log type, used a label in grafana
     * @param user          the current user, may be null if there is no user
     *                      context
     * @param keyValuePairs a list of key value pairs to output, must be an even
     *                      number of arguments or null
     */
