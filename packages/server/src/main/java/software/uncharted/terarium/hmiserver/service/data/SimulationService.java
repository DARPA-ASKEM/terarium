...
/**
 * Service class for handling simulations. Note that this does not extend TerariumAssetService, as
 * Simulations do not extend TerariumAsset. This is because simulations have special considerations
 * around their date/time fields when it comes to formatting.
 */
@Service
...
public class SimulationService {
  ...
  public List<Simulation> getSimulations(final Integer page, final Integer pageSize)
      throws IOException {
        ...
        new SearchRequest.Builder()
            ...
            .query(
                q ->
                    q.bool(
                        b ->
                            b.mustNot(mn -> mn.exists(e -> e.field("deletedOn")))
  ...
  public Optional<Simulation> getSimulation(final UUID id) throws IOException {
    final Simulation doc =
        elasticService.get(elasticConfig.getSimulationIndex(), id.toString(), Simulation.class);
    ...
    presigned.setUrl(
        s3ClientService
            ...
            .getS3PreSignedPutUrl(
                config.getFileStorageS3BucketName(), getPath(id, filename), HOUR_EXPIRATION));
