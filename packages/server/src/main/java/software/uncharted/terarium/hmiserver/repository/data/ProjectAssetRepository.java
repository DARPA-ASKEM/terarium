...
public interface ProjectAssetRepository extends PSCrudRepository<ProjectAsset, UUID> {
	...
	@Query(
			"SELECT p FROM ProjectAsset p WHERE p.assetId IN (SELECT p2.assetId FROM ProjectAsset p2 WHERE p2.deletedOn IS NULL GROUP BY p2.assetId HAVING COUNT(p2.assetId) > 1)")
