...
public class ClimateDataController {
  ...
  /**
   * @param esgfId The id of the ESGF (in the form
   *     `CMIP6.CMIP.NCAR.CESM2.historical.r11i1p1f1.CFday.ua.gn.v20190514`)
  ...
  public ResponseEntity<String> previewEsgf(
    ...
    try {
      final String previewResponse =
          climateDataService.getPreview(esgfId, variableId, timestamps, timeIndex);
      if (previewResponse != null) {
        return ResponseEntity.ok(previewResponse);
      }
      final boolean successfulFetch =
          climateDataService.fetchPreview(esgfId, variableId, timestamps, timeIndex);
      if (successfulFetch) {
    ...
    climateDataService.addPreviewJob(
        esgfId, variableId, timestamps, timeIndex, climateDataResponse.getId());
   ...
   * @param envelope Geographical envelope provided as a comma-separated series of 4 degrees: lon,
   *     lon, lat, lat. example `40,45,-80,-75`.
   * @param timestamps String of two ISO-8601 timestamps or the terms start or end separated by
   *     commas, example `start,2010-01-01T00:00:00`. Leave blank to generate for all times
   * @param thinFactor Take every nth datapoint along specified fields given by thin_fields. Leave
   *     blank for all data
   * @return
   */
  ...
  public ResponseEntity<String> subsetEsgf(
    ...
    try {
      response =
          climateDataProxy.subsetEsgf(esgfId, parentDatasetId, timestamps, envelope, thinFactor);
    } catch (final FeignException e) {
    ...
    climateDataService.addSubsetJob(
        esgfId, envelope, timestamps, thinFactor, climateDataResponse.getId());
