...
public class OntologyConceptRepositoryImpl implements OntologyConceptRepositoryCustom {
  ...
  @Autowired @Lazy private JPAQueryFactory queryFactory;
  ...
  public ConceptFacetSearchResponse facetQuery(List<TaggableType> types, List<String> curies) {
     ...
     * GROUP BY type
     */
    List<Tuple> typeCounts =
        query
            .select(ontologyConcept.type, ontologyConcept.objectId.countDistinct())
     ...
     */
    List<Tuple> conceptCounts =
        query
            .select(ontologyConcept.curie, activeConcept.name, ontologyConcept.count())
     ...
     * WHERE oc.type IN (%s)
     */
    List<Tuple> activeConceptResults =
        query
            .select(ontologyConcept, activeConcept.name)
            .leftJoin(activeConcept)
            .on(ontologyConcept.curie.eq(activeConcept.curie))
