...
public class ProvenanceSearchService {
  ...
  public ProvenanceSearchResult connectedNodes(final ProvenanceQueryParam payload) {
    try (final Session session = neo4jService.getSession()) {
      ...
      final String relationshipsStr =
          relationshipsArrayAsStr(
  ...
  private ProvenanceSearchResult connectedNodesByDirection(
      final ProvenanceQueryParam payload, final String direction) {
    try (final Session session = neo4jService.getSession()) {
      final String relationshipsStr =
          relationshipsArrayAsStr(
              Arrays.asList(ProvenanceRelationType.CONTAINS, ProvenanceRelationType.IS_CONCEPT_OF),
      ...
      final String relationDirection =
          dynamicRelationshipDirection(direction, String.format("r:'%s' *1..", relationshipsStr));
      final String matchNode = matchNodeBuilder(payload.getRootType(), payload.getRootId());
      final String nodeAbbr = returnNodeAbbr(payload.getRootType());

      final String query =
          String.format("'%s''%s'(n) return '%s', r, n", matchNode, relationDirection, nodeAbbr);
  ...
  /**
   * Identifies which model revisions help create the latest model which was used to create the
   * artifact.
   *
   * @param payload - Search param payload.
   * @return
  ...
  public ProvenanceSearchResult parentModelRevisions(final ProvenanceQueryParam payload) {
    if (!Arrays.asList(
            ...
            ProvenanceType.SIMULATION_RUN,
            ProvenanceType.PLAN,
            ProvenanceType.DATASET)
        .contains(payload.getRootType())) {
      throw new IllegalArgumentException(
          "Derived models can only be found from root types of Model, SimulationRun, Plan and"
              + " Dataset");
    }

    try (final Session session = neo4jService.getSession()) {
      final String matchPattern =
          parentModelQueryGenerator(payload.getRootType(), payload.getRootId());
      final String relationshipsStr =
          relationshipsArrayAsStr(
              Arrays.asList(ProvenanceRelationType.CONTAINS, ProvenanceRelationType.IS_CONCEPT_OF),
              new ArrayList<>());
      ...
      final String query =
          String.format(
              "'%s' OPTIONAL MATCH (Mr2:ModelRevision)-[r2:'%s' *1..]->(Mr) WITH *,"
                  + " COLLECT(r)+COLLECT(r2) AS r3, COLLECT(Mr)+COLLECT(Mr2) AS Mrs UNWIND Mrs AS"
                  + " Both_rms UNWIND r3 AS r4 WITH * OPTIONAL MATCH"
                  + " (Both_rms)<-[r5:BEGINS_AT]-(Md:Model) WITH *, COLLECT(r4)+COLLECT(r5) AS r6"
                  + " UNWIND r6 AS r7 RETURN Both_rms, Md, r7",
  ...
  public ProvenanceSearchResult parentModels(final ProvenanceQueryParam payload) {
    ...
    try (final Session session = neo4jService.getSession()) {
      ...
      final String modelRelationships =
          relationshipsArrayAsStr(
              new ArrayList<>(),
              Arrays.asList(
      ...
      final String query =
          String.format(
              "'%s' OPTIONAL MATCH (Mr)-[r2:'%s' *1..]->(Mr2:ModelRevision) WITH *,"
                  + " COLLECT(Mr)+COLLECT(Mr2) AS Mrs, COLLECT(r)+COLLECT(r2) AS r3 UNWIND Mrs AS"
                  + " Both_rms WITH * OPTIONAL MATCH (md2:Model)-[r4:BEGINS_AT]->(Both_rms) WITH *,"
                  + " COLLECT(r3)+COLLECT(r4) AS r5 RETURN '%s', md2, Both_rms, r5",
  ...
  public ProvenanceSearchResult artifactsCreatedByUser(final ProvenanceQueryParam payload) {
    try (final Session session = neo4jService.getSession()) {
      ...
      final String query =
          String.format(
              "'%s'-[r]->(n2) WHERE r.user_id='%s' WITH *, COLLECT(n)+COLLECT(n2) AS nodes UNWIND"
                  + " nodes AS both_nodes WITH * RETURN both_nodes",
  ...
  public Set<String> modelsFromCode(final ProvenanceQueryParam payload) {
    if (payload.getRootType() != ProvenanceType.MODEL) {
      throw new IllegalArgumentException(
          "Code used for model extraction can only be found by providing a Model");
    }
    ...
    try (final Session session = neo4jService.getSession()) {
      ...
      final String query =
          String.format(
              "MATCH (c:Code)<-[r:EXTRACTED_FROM]-(m:Model {id: '%s'}) RETURN c", modelId);
  ...
  public Set<String> modelsFromEquation(final ProvenanceQueryParam payload) {
    ...
    try (final Session session = neo4jService.getSession()) {
      ...
      final String query =
          String.format(
              "MATCH (e:Equation)<-[r:EXTRACTED_FROM]-(m:Model {id: '%s'}) RETURN e", modelId);
  ...
  public Set<String> modelConfigFromDocument(final ProvenanceQueryParam payload) {
    if (payload.getRootType() != ProvenanceType.MODEL_CONFIGURATION) {
      throw new IllegalArgumentException(
          "Document used for model-configuration extraction can only be found by providing a"
  ...
  public Set<String> modelConfigFromDataset(final ProvenanceQueryParam payload) {
    if (payload.getRootType() != ProvenanceType.MODEL_CONFIGURATION) {
      throw new IllegalArgumentException(
          "Dataset used for model-configuration extraction can only be found by providing a"
              + " model-confirguration");
    }
  ...
  public Set<String> modelsFromDocument(final ProvenanceQueryParam payload) {
    ...
    try (final Session session = neo4jService.getSession()) {
      ...
      final String query =
          String.format(
              "MATCH (d:Document)<-[r:EXTRACTED_FROM]-(m:Model {id: '%s'}) RETURN d", modelId);
  ...
  public Map<String, Integer> conceptCounts(final ProvenanceQueryParam payload) {
    try (final Session session = neo4jService.getSession()) {
      ...
      final String query =
          String.format(
              "'%s'-[r:IS_CONCEPT_OF]->(n) WHERE Cn.concept='%s' RETURN labels(n) as label, n.id as"
                  + " id",
              matchNode, payload.getCurie());
  ...
  public Map<String, Object> modelDocument(final ProvenanceQueryParam payload) {
    try (final Session session = neo4jService.getSession()) {
      final String query =
          String.format(
              "MATCH (Md:Model {id:'%s'})<-[r:REINTERPRETS|EXTRACTED_FROM|BEGINS_AT"
                  + " *1..]->(Do:Document) RETURN Do",
              payload.getRootId());
  ...
  public ProvenanceSearchResult nodesEdges(
      final Result response, final ProvenanceQueryParam payload) {
    ...
    final List<ProvenanceType> types =
        payload.getTypes() != null ? payload.getTypes() : new ArrayList<>();
  ...
  public static String dynamicRelationshipDirection(
      final String direction, final String relationshipType) {
    switch (direction) {
      case "all":
  ...
  public static String extractedModelsQueryGenerator(
      final ProvenanceType rootType, final UUID rootId) {
    switch (rootType) {
      case DOCUMENT:
        return String.format(
            "MATCH (m:Model)-[:EXTRACTED_FROM]->(d:Document {id:'%s'}) RETURN m", rootId);
      case CODE:
        return String.format(
            "MATCH (m:Model)-[:EXTRACTED_FROM]->(c:Code {id:'%s'}) RETURN m", rootId);
      case EQUATION:
        return String.format(
            "MATCH (m:Model)-[:EXTRACTED_FROM]->(e:Equation {id:'%s'}) RETURN m", rootId);
      default:
  ...
  public String parentModelQueryGenerator(final ProvenanceType rootType, final UUID rootId) {
    ...
    queryTemplatesIndex.put(
        ProvenanceType.MODEL, String.format("-[r:BEGINS_AT]->'%s' ", modelRevisionNode));
    queryTemplatesIndex.put(
        ProvenanceType.MODEL_CONFIGURATION, String.format("-[r:USES]->'%s' ", modelRevisionNode));
    queryTemplatesIndex.put(
  ...
  public String returnNodeAbbr(final ProvenanceType nodeType) {
    final Map<ProvenanceType, String> provenanceTypeToAbbr =
        new HashMap<>() {
          {
