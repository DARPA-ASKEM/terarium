...
public class ProvenanceSearchService {
    ...
    /**
     * Identifies which model revisions help create the latest model which was used to create the artifact.
     *
     * @param payload - Search param payload.
     * @return
     */
    ...
    public Set<String> modelsFromCode(final ProvenanceQueryParam payload) {
        ...
        try (final Session session = neo4jService.getSession()) {
            ...
            final String query =
                    String.format("MATCH (c:Code)<-[r:EXTRACTED_FROM]-(m:Model {id: '%s'}) RETURN c", modelId);
    ...
    public Set<String> modelsFromEquation(final ProvenanceQueryParam payload) {
        ...
        try (final Session session = neo4jService.getSession()) {
            ...
            final String query =
                    String.format("MATCH (e:Equation)<-[r:EXTRACTED_FROM]-(m:Model {id: '%s'}) RETURN e", modelId);
    ...
    public String parentModelQueryGenerator(final ProvenanceType rootType, final UUID rootId) {
        ...
        queryTemplatesIndex.put(
                ProvenanceType.SIMULATION, String.format("-[r:'%s' *1..]->'%s' ", relationshipsStr, modelRevisionNode));
        queryTemplatesIndex.put(
                ProvenanceType.DATASET, String.format("-[r:'%s' *1..]->'%s' ", relationshipsStr, modelRevisionNode));
