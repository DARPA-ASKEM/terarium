...
public abstract class TerariumAssetServiceWithoutSearch<T extends TerariumAsset, R extends PSCrudSoftDeleteRepository<T, UUID>>
		...
		try {
			...
			for (final String fileName : cloned.getFileNames()) {
				...
				try {
					...
					final ResponseInputStream<GetObjectResponse> stream = s3ClientService.getS3Service().getObject(
							bucket,
							key);
	...
	public Optional<String> fetchFileAsString(final UUID uuid, final String filename) throws IOException {
		...
		try {
			final ResponseInputStream<GetObjectResponse> stream = s3ClientService.getS3Service().getObject(bucket,
					key);
