...
public class ProvenanceService {
  ...
  public Map<String, List<List<String>>> loadGraphValidations() throws Exception {
    ...
    Map<String, List<List<String>>> jsonMap =
        objectMapper.readValue(new InputStreamReader(inputStream), Map.class);
    return jsonMap;
  }

  private boolean validateRelationship(
      ProvenanceType left, ProvenanceType right, ProvenanceRelationType relationType) {
  ...
  public Provenance createProvenance(Provenance provenance) {
    ...
    try (Session session = neo4jService.getSession()) {
      ...
      String edgeQuery =
          String.format(
              "MATCH (n1:%s {id: $left_id}) MATCH (n2:%s {id: $right_id}) MERGE (n1)-[:%s {user_id:"
                  + " $user_id, provenance_id: $provenance_id}]->(n2)",
              provenance.getLeftType(), provenance.getRightType(), provenance.getRelationType());
      session.run(
          edgeQuery,
          Values.parameters(
  ...
  public void deleteProvenance(UUID id) {
    ...
    try (Session session = neo4jService.getSession()) {
      String query =
          String.format(
              "MATCH (n1:%s {id: $left_id}) MATCH (n2:%s {id: $right_id}) MATCH (n1)-[r:%s]->(n2)"
                  + " DELETE r",
