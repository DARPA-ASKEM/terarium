...
public class AssetController {
  ...
  /**
   * Check if an asset name is available for a given asset type. If a ProjectId is given the search
   * will be limited to just that project. Otherwise, the entire asset type will be searched. If the
   * asset name is available, a 204 No Content response is returned. If the asset name is not
   * available, a 409 Conflict response is returned.
   *
   ...
   * @return 204 No Content if the asset name is available, 409 Conflict if the asset name is not
   *     available
   */
  ...
  @Operation(
      summary =
          "Check if an asset name is available for a given asset type. If a ProjectId is given the"
              + " search will be limited to just that project. Otherwise, the entire asset type"
              + " will be searched.")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<Void> verifyAssetNameAvailability(
    ...
    if (projectId == null) {
      ...
      final Optional<ProjectAsset> asset =
          projectAssetService.getProjectAssetByNameAndType(assetName, assetType);
      if (asset.isPresent()) {
