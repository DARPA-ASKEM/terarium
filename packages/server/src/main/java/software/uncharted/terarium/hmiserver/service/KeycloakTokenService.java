...
public class KeycloakTokenService {
  ...
  /**
   * Get a token from keycloak. If the token is cached, it will be returned. If the token is
   * expired, it will be refreshed and returned.
   *
   ...
   *
   * @param token the token to test for validity. Obtained from the access_token field of the
  ...
  public boolean hasRealmRole(String token, String role) {
    try {
      ...
      return ((List<String>)
              jwt.getClaimAsMap("realm_access").getOrDefault("roles", new ArrayList<>()))
          .contains(role);
    } catch (Exception e) {
      log.error("Error decoding token", e);
      return false;
