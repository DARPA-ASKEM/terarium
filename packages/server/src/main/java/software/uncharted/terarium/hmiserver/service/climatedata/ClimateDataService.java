...
public class ClimateDataService {
    ...
    for (final ClimateDataPreviewTask previewTask : previewTasks) {
      ...
      if (response.getBody() != null && response.getBody() instanceof final IntNode intNode) {
        if (intNode.intValue() >= 400) {
          ...
          final ClimateDataPreview preview =
              new ClimateDataPreview(previewTask, "Failed to extract PNG");
      ...
      if (!climateDataResponse.getResult().getJobResult().isNull()) {
        final ClimateDataResultPreviews previews =
        ...
        if (previews != null
          ...
          try {
            ...
            if (index > -1 && index + 1 < preview.getImage().length()) {
              ...
              final String key =
                  getPreviewFilename(previewTask.getEsgfId(), previewTask.getVariableId());
      ...
      if (!climateDataResponse.getResult().getJobError().isNull()) {
        final ClimateDataPreview preview =
            new ClimateDataPreview(previewTask, climateDataResponse.getResult().getJobError());
        climateDataPreviewRepository.save(preview);
  ...
  public void checkSubsetTaskStatus() {
    ...
    for (final ClimateDataSubsetTask subsetTask : subsetTasks) {
      ...
      if (!climateDataResponse.getResult().getJobResult().isNull()) {
        try {
          final ClimateDataResultSubset result =
              objectMapper.convertValue(
          ...
          if (result.getStatus().equals("failed")) {
            ...
            final ClimateDataSubset subset =
                new ClimateDataSubset(subsetTask, result.getDatasetId());

            climateDataSubsetRepository.save(subset);
          } else {
            log.error(
                "Unexpected status extract subset: "
                    + climateDataResponse.getResult().getJobResult());
            final ClimateDataSubset subset =
      ...
      if (!climateDataResponse.getResult().getJobError().isNull()) {
        final ClimateDataSubset subset =
            new ClimateDataSubset(subsetTask, climateDataResponse.getResult().getJobError());
        climateDataSubsetRepository.save(subset);
