...
export const getXDDFacets = (articles: XDDArticle[], filters?: string[]) => {
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export const getDatacubeFacets = (articles: Datacube[], filters?: string[]) => {
	const facets = {} as Facets;
	const aggField = (fieldName: string) => {
		const aggs: FacetBucket[] = [];
...
export const getFacets = (results: SearchResults[], resultType: string, filters?: string[]) => {
