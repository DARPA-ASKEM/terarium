...
/**
 * Terarium modifications
 *
 * - Adding a "obstacles" option to route amongst all nodes
...
function computePt(p: ConnectorPoint): Point {
	...
	switch (p.side) {
		...
		default:
			throw new Error(`unknown ${p.side}`);
	}
}
...
function extrudeCp(cp: ConnectorPoint, margin: number): Point {
	...
	switch (cp.side) {
		...
		case 'left':
			return makePt(x - margin, y);
		default:
			throw new Error(`unknown ${cp.side}`);
	}
}
 ...
 */
export default class OrthogonalConnector {
	...
	static route(opts: OrthogonalConnectorOpts): Point[] {
		...
		const obstableRectangles = opts.obstacles ? opts.obstacles.map((d) => Rectangle.fromRect(d)) : [];
		...
		// Add edges to rulers
		// eslint-disable-next-line
		for (const b of [inflatedA, inflatedB, ...obstableRectangles]) {
		...
		for (const connectorPt of [pointA, pointB]) {
			...
			switch (connectorPt.side) {
				...
				case 'left':
					...
					break;
				default:
					console.error(`cannot find ${connectorPt.side}`);
			}
		}
		...
		const gridPoints = gridToSpots(grid, [inflatedA, inflatedB, ...obstableRectangles]);
