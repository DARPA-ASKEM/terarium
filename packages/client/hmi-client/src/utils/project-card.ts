		...
		for (let iY = 10; iY < imageHeight; iY += spacerY) {
			for (let iX = imageWidth / 2; iX < imageWidth && iX > -imageWidth; iX += spacerX * side) {
				...
				for (let h = dotSize; h > 2; h--) {
					p.fill(
						p.lerpColor(
							p.lerpColor(from, to, iY / imageHeight),
							p.color(0, 0, 100, 0.1),
							(1 - h / dotSize) * 0.25
						)
					);
...
export default function (args: Iargs): string {
	...
	models = args.models;
	datasets = args.models;
	papers = args.papers;

	// Create the p5 sketch
	const p = new p5(sketch); /* eslint-disable-line new-cap */

	// Add all of this within the canvas and export the base64 image
	const canvas = p.createCanvas(imageWidth, imageHeight) as Renderer;
