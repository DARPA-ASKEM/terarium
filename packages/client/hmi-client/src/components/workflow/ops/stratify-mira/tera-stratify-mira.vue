...
const handleModelPreview = async (data: any) => {
	...
	// Create output
	const projectAsset: ProjectAsset | null = await createModel(amrResponse);
	if (!projectAsset) return;
	const modelData: Model | null = await getModel(projectAsset.assetId);
	if (!modelData) {
		logger.error('Error getting updated model');
		return;
	}
