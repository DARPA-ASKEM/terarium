...
const createOutput = async (modelToSave: Model) => {
	...
	const name = 'assetName' in projectAsset ? projectAsset.assetName : projectAsset.name;
	const id = 'assetId' in projectAsset ? projectAsset.assetId : projectAsset.id;

	const modelLabel = isReadyToCreateDefaultOutput.value ? (name as string) : nodeOutputLabel(props.node, 'Output'); // Just label the original model with its name
