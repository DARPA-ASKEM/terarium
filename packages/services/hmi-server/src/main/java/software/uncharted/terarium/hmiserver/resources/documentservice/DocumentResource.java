...
public class DocumentResource {
	...
	// NOTE: the query parameters match the proxy version and the type
	// XDDSearchPayload
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	...
	public Response getDocuments(
		...
		if (docid != null || doi != null || term != null || github_url != null || similar_to != null) {
			// for a more direct search, if doi is valid, then make sure other params are
			// null
			...
			try {
				...
				XDDResponse<DocumentsResponseOK> doc = proxy.getDocuments(apiKey,
						docid, doi, title, term, dataset, include_score, include_highlights, inclusive, full_results,
						max, per_page, dict, facets,
						min_published, max_published, pubname, publisher, additional_fields, match, known_entities,
						github_url, similar_to);
